...
fn make_plan(name: &str, inputs: Vec<(&str, &str)>, steps: Vec<CompiledStep>) -> (CompiledPlan, PlanDef) {
    let plan_inputs: Vec<cadmus::plan::PlanInput> = inputs.iter()
        .map(|(k, _v)| cadmus::plan::PlanInput::bare(*k))
        .collect();
    let compiled = CompiledPlan {
        name: name.to_string(),
        input_type: TypeExpr::prim("Any"),
        input_description: inputs.first().map(|(k, _)| k.to_string()).unwrap_or_default(),
        steps,
        output_type: TypeExpr::prim("Any"),
    };
    let def = PlanDef {
