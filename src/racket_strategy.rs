...
//
// Architecture:
//   - `build_racket_registry()` — loads racket.ops.yaml
...
pub fn build_racket_registry() -> OperationRegistry {
    if let Ok(reg) = crate::registry::load_ops_pack("data/packs/ops/racket.ops.yaml") {
        return reg;
    }
    load_ops_pack_str(RACKET_OPS_YAML)
        .expect("embedded racket.ops.yaml should always parse")
}
...
pub fn load_racket_facts() -> Result<FactPackIndex, InferenceError> {
    let path = Path::new("data/packs/facts/racket.facts.yaml");
