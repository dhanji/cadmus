...
impl ComparisonStrategy {
    pub fn new(goal: Goal) -> Result<Self> {
        let paths: Vec<std::path::PathBuf> = goal
            .fact_pack_paths
            .iter()
            .map(std::path::PathBuf::from)
            .collect();
        let index = fact_pack::load_fact_packs(&paths)?;
...
impl ReasonerStrategy for ComparisonStrategy {
    ...
    fn build_registry(&self) -> OperationRegistry {
        ...
        const COMPARISON_OPS_YAML: &str = include_str!("../data/packs/ops/comparison.ops.yaml");
        crate::registry::load_ops_pack_str_into(COMPARISON_OPS_YAML, &mut reg)
            .expect("embedded comparison.ops.yaml should always parse");
